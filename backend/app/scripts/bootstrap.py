"""初始数据引导（幂等）：默认项目 + 项目负责人账号与成员身份。

首版不开放公开注册（16 节），新环境通过本脚本获得首个可登录账号，
并把它登记为默认项目（2.2 节单项目）的 leader 成员。
...
async def ensure_admin_membership(project: Project, user: User) -> bool:
    """幂等把初始账号登记为默认项目的 leader 成员。返回是否新建。"""
    async with async_session_factory() as session:
        existing = (
            await session.execute(
...
async def main() -> None:
    ...
    logger.info(
        "bootstrap: project=%s, admin=%s, leader membership %s",
