"""0008：deliverables 与 reviews（交付物版本化与最终审核，7.5、11 章）。
...
- deliverables：三类交付物（git_link/text/file）按工作项版本递增留痕；
  (work_item_id, version) 唯一约束兜底并发重提（17.2 节），旧版本保留可查；
  file 类型经 stored_file_id 关联 stored_files，可追溯 sha256（2.1 节）。
- reviews：负责人最终审核留痕（结论、反馈、被审交付物版本、审核人）。
- collaboration_requests 增加可空 result_deliverable_id / result_file_id：
  协作产物回传可引用交付物/文件（T4.4，7.2 节）。
"""
...
def upgrade() -> None:
    op.create_table(
        ...
        # 版本号：同一工作项内从 1 递增，重提不覆盖旧版本（7.5 节）
    ...
    op.create_table(
        ...
        # 反馈正文：隐私信息，仅负责人与该工作项主执行人可见（16 节）
