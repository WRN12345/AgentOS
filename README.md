...
| 服务 | 职责 |
| --- | --- |
| `frontend` | React 18 + TypeScript + Vite + shadcn/ui，TanStack Query + Zustand 状态管理 |
| `backend` | Python 3.12 + FastAPI 模块化单体，SQLAlchemy Async + Alembic 迁移 |
| `worker` | 异步任务消费者：Agent 运行、记忆索引、失败重试 |
| `scheduler` | 定时扫描：到期提醒、逾期预警、周期风险 |
| `postgres` | PostgreSQL 16 + pgvector，业务数据与向量检索 |
| `redis` | 任务队列与延迟调度 |
